...
export async function PATCH(
  ...
  let body: EnabledBody;
  try {
    body = (await request.json()) as EnabledBody;
  } catch {
  ...
  const { data, error } = await supabase.rpc("update_strategy_field_optimistic", {
    p_strategy_id: strategyId,
    p_field_name: "enabled",
    p_field_value: body.enabled,
